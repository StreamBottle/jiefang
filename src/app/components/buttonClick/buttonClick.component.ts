    ...
    for (let key in eventObject) {
      if (eventObject.hasOwnProperty(key)) {
        ...
        this.nativeElement.addEventListener(key, (e) => {
          ...
          if (this.data.id === 'page01-returnPricinple' && this.data.currentStateID === 'state01') {
            $('.redCircle').show();
            this.appService.returnPricinple = false;
            $('.textInfoBox').hide();
            $('.theory').hide();
            setTimeout(function () {
            ...
            $('.animate-pic').hide();
            $('.zhizhen-pic').hide();
            this.appService.showZhizhenAnimatePic = false;
